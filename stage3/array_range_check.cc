...
void array_range_check_c::check_bounds(array_variable_c *symbol) {
  ...
  for (int i =  0; i < l->n; i++) {
    ...
    /* Check lower limit */
    if ( VALID_CVALUE( int64_iec, l->get_element(i)) && VALID_CVALUE( int64_iec, dimension->lower_limit))
      if ( GET_CVALUE( int64_iec, l->get_element(i)) < GET_CVALUE( int64_iec, dimension->lower_limit) )
      {STAGE3_ERROR(0, symbol, symbol, "Array access out of bounds (using constant value of %"PRId64", should be >= %"PRId64").", GET_CVALUE( int64_iec, l->get_element(i)), GET_CVALUE( int64_iec, dimension->lower_limit)); continue;}

    if ( VALID_CVALUE( int64_iec, l->get_element(i)) && VALID_CVALUE(uint64, dimension->lower_limit))
      if ( cmp_unsigned_signed( GET_CVALUE(uint64, dimension->lower_limit), GET_CVALUE( int64_iec, l->get_element(i))) > 0 )
      {STAGE3_ERROR(0, symbol, symbol, "Array access out of bounds (using constant value of %"PRId64", should be >= %"PRIu64").", GET_CVALUE( int64_iec, l->get_element(i)), GET_CVALUE(uint64, dimension->lower_limit)); continue;}
    ...
    if ( VALID_CVALUE(uint64, l->get_element(i)) && VALID_CVALUE( int64_iec, dimension->lower_limit))
      if ( cmp_unsigned_signed(GET_CVALUE(uint64, l->get_element(i)), GET_CVALUE( int64_iec, dimension->lower_limit)) < 0 )
      {STAGE3_ERROR(0, symbol, symbol, "Array access out of bounds (using constant value of %"PRIu64", should be >= %"PRId64").", GET_CVALUE(uint64, l->get_element(i)), GET_CVALUE( int64_iec, dimension->lower_limit)); continue;}

    /* Repeat the same check, now for upper limit */
    if ( VALID_CVALUE( int64_iec, l->get_element(i)) && VALID_CVALUE( int64_iec, dimension->upper_limit))
      if ( GET_CVALUE( int64_iec, l->get_element(i))   >  GET_CVALUE( int64_iec, dimension->upper_limit))
      {STAGE3_ERROR(0, symbol, symbol, "Array access out of bounds (using constant value of %"PRId64", should be <= %"PRId64").", GET_CVALUE( int64_iec, l->get_element(i)), GET_CVALUE( int64_iec, dimension->upper_limit)); continue;}

    if ( VALID_CVALUE( int64_iec, l->get_element(i)) && VALID_CVALUE(uint64, dimension->upper_limit))
      if ( cmp_unsigned_signed( GET_CVALUE(uint64, dimension->upper_limit), GET_CVALUE( int64_iec, l->get_element(i))) < 0 )
      {STAGE3_ERROR(0, symbol, symbol, "Array access out of bounds (using constant value of %"PRId64", should be <= %"PRIu64").", GET_CVALUE( int64_iec, l->get_element(i)), GET_CVALUE(uint64, dimension->upper_limit)); continue;}
    ...
    if ( VALID_CVALUE(uint64, l->get_element(i)) && VALID_CVALUE( int64_iec, dimension->upper_limit))
      if ( cmp_unsigned_signed(GET_CVALUE(uint64, l->get_element(i)), GET_CVALUE( int64_iec, dimension->upper_limit)) > 0 )
      {STAGE3_ERROR(0, symbol, symbol, "Array access out of bounds (using constant value of %"PRIu64", should be <= %"PRId64").", GET_CVALUE(uint64, l->get_element(i)), GET_CVALUE( int64_iec, dimension->upper_limit)); continue;}
...
void *array_range_check_c::visit(subrange_c *symbol) {
	...
	if        (VALID_CVALUE( int64_iec, symbol->upper_limit) && VALID_CVALUE( int64_iec, symbol->lower_limit)) {  
		...
		// dimension = GET_CVALUE( int64_iec, symbol->upper_limit) - VALID_CVALUE( int64_iec, symbol->lower_limit);
		if  (GET_CVALUE( int64_iec, symbol->lower_limit)  >   GET_CVALUE( int64_iec, symbol->upper_limit)) {
			STAGE3_ERROR(0, symbol, symbol, "Subrange has lower limit (%"PRId64") larger than upper limit (%"PRId64").", GET_CVALUE( int64_iec, symbol->lower_limit), GET_CVALUE( int64_iec, symbol->upper_limit));
			dimension = std::numeric_limits< unsigned long long int >::max() - 1; // -1 because it will be incremented at the end of this function!! 
		} else if (GET_CVALUE( int64_iec, symbol->lower_limit) >= 0) {
			dimension = GET_CVALUE( int64_iec, symbol->upper_limit) - GET_CVALUE( int64_iec, symbol->lower_limit);
		} else {
			dimension  = -GET_CVALUE( int64_iec, symbol->lower_limit);
			dimension +=  GET_CVALUE( int64_iec, symbol->upper_limit);     
		}
	...
	} else if (VALID_CVALUE(uint64, symbol->upper_limit) && VALID_CVALUE( int64_iec, symbol->lower_limit)) {
		// No need to check whether lower_limit > upper_limit, as we only reach this point if lower_limit < 0 (and upper_limit must be >= 0!)
		if (GET_CVALUE( int64_iec, symbol->lower_limit) >= 0) {
			dimension = GET_CVALUE(uint64, symbol->upper_limit) - GET_CVALUE( int64_iec, symbol->lower_limit);
		} else {
			unsigned long long int lower_ull;
			lower_ull  = -GET_CVALUE( int64_iec, symbol->lower_limit);
	...
	} else if (!VALID_CVALUE(uint64, symbol->upper_limit) && !VALID_CVALUE( int64_iec, symbol->upper_limit)) {
		...
		dimension = std::numeric_limits< unsigned long long int >::max() - 1; // -1 because it will be incremented at the end of this function!! 
	} else if (!VALID_CVALUE(uint64, symbol->lower_limit) && !VALID_CVALUE( int64_iec, symbol->lower_limit)) {
...
void *array_range_check_c::visit(array_initial_elements_c *symbol) {
	if (VALID_CVALUE( int64_iec, symbol->integer) && (GET_CVALUE( int64_iec, symbol->integer) < 0)) 
