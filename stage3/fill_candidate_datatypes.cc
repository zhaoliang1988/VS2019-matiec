...
void fill_candidate_datatypes_c::remove_incompatible_datatypes(symbol_c *symbol) {
  ...
  {/* Remove signed data types */
    int64_t value = 0;
    if (VALID_CVALUE(  int64_iec, symbol)) value = GET_CVALUE(int64_iec, symbol);
    if (IS_OVERFLOW (  int64_iec, symbol)) value = (int64_t)INT32_MAX + (int64_t)1;
    ...
    if (IS_OVERFLOW( int64_iec, symbol))                {__REMOVE__(lint_type_name);}
  }
...
void *fill_candidate_datatypes_c::visit(enumerated_value_c *symbol) {
	...
	if (NULL != symbol->type) {
		...
		if (local_enumerated_value_symtable.find(symbol->value) == local_enumerated_value_symtable.end())
		{
