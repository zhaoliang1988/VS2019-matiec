...
static void CHECK_OVERFLOW_int64_SUM(symbol_c *res, symbol_c *a_ptr, symbol_c *b_ptr) {
	if (!VALID_CVALUE(int64_iec, res))
		return;
	int64_t a = GET_CVALUE(int64_iec, a_ptr);
	int64_t b = GET_CVALUE(int64_iec, b_ptr);
	/* The following test is valid no matter what representation is being used (e.g. two's complement, etc...) */
	if (((b > 0) && (a > (INT64_MAX - b)))
	 || ((b < 0) && (a < (INT64_MIN - b))))
		SET_OVFLOW(int64_iec, res);
}
...
static void CHECK_OVERFLOW_int64_NEG(symbol_c *res, symbol_c *a) {
	if (!VALID_CVALUE(int64_iec, res))
		return;
	if (GET_CVALUE(int64_iec, a) == INT64_MIN)
		SET_OVFLOW(int64_iec, res);
}
 ...
 *   The result of comparig the bool, int64_iec, and uint64 are overwritten by the comparison of the real64 type!
 */
...
#define handle_cmp(symbol, oper1, oper2, operation) {               \
	...
	DO_BINARY_OPER( int64_iec, operation, bool, oper1, oper2);     \
...
static void *handle_neg(symbol_c *symbol, symbol_c *oper) {
	...
	DO_UNARY_OPER( int64_iec, -, oper);	CHECK_OVERFLOW_int64_NEG (symbol, oper);
...
static void *handle_add(symbol_c *symbol, symbol_c *oper1, symbol_c *oper2) {
	...
	DO_BINARY_OPER( int64_iec, +,  int64_iec, oper1, oper2);   CHECK_OVERFLOW_int64_SUM (symbol, oper1, oper2);
...
static void *handle_sub(symbol_c *symbol, symbol_c *oper1, symbol_c *oper2) {
	...
	DO_BINARY_OPER( int64_iec, -,  int64_iec, oper1, oper2);   CHECK_OVERFLOW_int64_SUB (symbol, oper1, oper2);
	DO_BINARY_OPER(real64, -, real64, oper1, oper2);   CHECK_OVERFLOW_real64    (symbol);
	return NULL;
}
...
static void *handle_mul(symbol_c *symbol, symbol_c *oper1, symbol_c *oper2) {
	...
	DO_BINARY_OPER( int64_iec, *,  int64_iec, oper1, oper2);   CHECK_OVERFLOW_int64_MUL (symbol, oper1, oper2);
	DO_BINARY_OPER(real64, *, real64, oper1, oper2);   CHECK_OVERFLOW_real64    (symbol);
	return NULL;
}
...
static void *handle_div(symbol_c *symbol, symbol_c *oper1, symbol_c *oper2) {
	...
	if (ISZERO_CVALUE( int64_iec, oper2))  {SET_OVFLOW( int64_iec, symbol);} else {DO_BINARY_OPER( int64_iec, /,  int64_iec, oper1, oper2); CHECK_OVERFLOW_int64_DIV (symbol, oper1, oper2);};
...
static void *handle_mod(symbol_c *symbol, symbol_c *oper1, symbol_c *oper2) {
	...
	if (ISZERO_CVALUE( int64_iec, oper2))  {SET_CVALUE( int64_iec, symbol, 0);} else {DO_BINARY_OPER( int64_iec, %,  int64_iec, oper1, oper2); CHECK_OVERFLOW_int64_MOD (symbol, oper1, oper2);};
	return NULL;
}


static void *handle_pow(symbol_c *symbol, symbol_c *oper1, symbol_c *oper2) {
	/* NOTE: If the const_value in symbol->r_exp is within the limits of both int64_iec and uint64, then we do both operations.
	 ...
	 */
	if (VALID_CVALUE(real64, oper1) && VALID_CVALUE( int64_iec, oper2))
		SET_CVALUE(real64, symbol, pow(GET_CVALUE(real64, oper1), GET_CVALUE( int64_iec, oper2)));
...
static void intersect_prev_cvalues(il_instruction_c *symbol) {
	...
	intersect_prev_CVALUE_( int64_iec, symbol);
...
void *constant_folding_c::visit(integer_c *symbol) {
	bool overflow;
	SET_CVALUE( int64_iec, symbol, extract_int64_value (symbol, &overflow));
	if (overflow) SET_OVFLOW(int64_iec, symbol);
...
void *constant_folding_c::visit(neg_integer_c *symbol) {
	...
	DO_UNARY_OPER( int64_iec, -, symbol->exp); CHECK_OVERFLOW_int64_NEG (symbol, symbol->exp);
	if (IS_OVFLOW( int64_iec, symbol->exp)) SET_OVFLOW( int64_iec, symbol);
	 ...
	 * However, the positive value cannot be stored inside an int64_iec! So, in this case, we will get the value from the uint64 cvalue.
	 *
	...
	if (VALID_CVALUE(uint64, symbol->exp) && (GET_CVALUE(uint64, symbol->exp) == (uint64_t)INT64_MAX+1)) {
		SET_CVALUE(int64_iec, symbol, INT64_MIN);
	}
	return NULL;
}
...
void *constant_folding_c::visit(integer_literal_c *symbol) {
	...
	DO_UNARY_OPER( int64_iec, /* none */, symbol->value);
...
static constant_propagation_c::map_values_t inner_left_join_values(constant_propagation_c::map_values_t m1, constant_propagation_c::map_values_t m2) {
	...
	for ( ; itr != m1.end(); ++itr) {
		...
		if (m2.count(name) > 0) {
			...
			COMPUTE_MEET_SEMILATTICE ( int64_iec, c1, c2, value);
...
void *constant_propagation_c::visit(external_declaration_c *symbol) {
  ...
  if (NULL != current_configuration) {
     ...
     * been set previously!
     */
    if (   !IS_UNDEFINED( int64_iec, symbol->specification) || !IS_UNDEFINED(uint64, symbol->specification) 
...
void *constant_propagation_c::visit(for_statement_c *symbol) {
	...
	(*values)[get_var_name_c::get_name(symbol->control_variable)->value].int64_iec.status = const_value_c::cs_non_const;

	/* Optimize dead code */
	if (NULL != symbol->by_expression) {
		symbol->by_expression->accept(*this);
		if (VALID_CVALUE(int64_iec, symbol->by_expression ) &&   GET_CVALUE(int64_iec, symbol->by_expression ) > 0 &&
			VALID_CVALUE(int64_iec, symbol->beg_expression) && VALID_CVALUE(int64_iec, symbol->end_expression)     &&
			  GET_CVALUE(int64_iec, symbol->beg_expression) >    GET_CVALUE(int64_iec, symbol->end_expression))
			return NULL;
		...
		if (VALID_CVALUE(int64_iec, symbol->beg_expression) && VALID_CVALUE(int64_iec, symbol->end_expression)     &&
			  GET_CVALUE(int64_iec, symbol->beg_expression) >    GET_CVALUE(int64_iec, symbol->end_expression))
			return NULL;
