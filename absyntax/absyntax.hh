...
class const_value_c {
  public:
    ...
    const_value__< int64_t>  _int64_iec; /* status is initialised to UNDEFINED */
    ...
    bool operator==(const const_value_c cv)
      {return ((_int64_iec==cv._int64_iec) && (_uint64==cv._uint64) && (_real64==cv._real64) && (_bool==cv._bool));}                                                     
      
    /* return true if at least one of the const values (int, real, ...) is a valid const value */
    bool is_const(void) 
      {return (_int64_iec.is_valid() || _uint64.is_valid() || _real64.is_valid() || _bool.is_valid());}   
};
