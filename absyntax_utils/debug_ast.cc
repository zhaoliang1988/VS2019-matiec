...
static void dump_cvalue(const_value_c const_value) {
  ...
  fprintf(stderr, ", i=");
#ifdef _WIN64
  if (const_value._int64_iec.is_valid()) fprintf(stderr, "%""lld""", const_value._int64_iec.get());
#else
  if      (const_value. _int64_iec.is_valid   ()) fprintf(stderr, "%"PRId64"", const_value. _int64_iec.get());
#endif
  else if (const_value. _int64_iec.is_overflow()) fprintf(stderr, "ov");
  else if (const_value. _int64_iec.is_nonconst()) fprintf(stderr, "nc");
  else                                        fprintf(stderr, "?");
  fprintf(stderr, ", u=");
#ifdef _WIN64
  if (const_value._uint64.is_valid()) fprintf(stderr, "%""llu""", const_value._uint64.get());
#else
